...
Then we can create a class which 'inherits from the 'abstract base class'.
...
=test.py= is the entry point for this project. (The contents of this file are shown at the end)
...
- Given the file tree shown above, =module_1.py=, =module_2.py= and =test.py= are single-file modules and have the standard attributes of a module as well as anything else we define in that module.
...
If a file, or more accurately a module, is found in the same directory, it can simply be imported as is.
...
As discussed above =__init__.py= is a powerful initialisation tool for our packages.
...
The obvious problem with =package_2/= is that nested modules are not accessible from the imported symbol.
...
- the =in= operator is used to check if the symbols were in the scope / bound to a package
- a =True= result means that the symbol was found
...
Python code generally follows a single thread of execution. If /IO/ bound operations are taking the majority of time in a program, concurrency can be used as in javascript. In this type of programming, multiple threads which are serviced independently give the illusion of things happening at the same time.

If a program is /CPU/ intensive, multiprocessing should be used. This will side-step the /Global Interpreter Lock/ and allow multiple instances of python to process data over all processor cores.
...
This function is not computationally heavy, but it takes a long time, about 20 seconds. The sleep function simulates something like an /API/ request that must be waited for. If we want to trigger the function multiple times, it would be a good idea to use =threading= to start these threads in the background and service them with processor time only when required.
...
Rather than waiting for something, the function is performing calculations. We can use multiple cores on our machine to do the processing simultaneously. This is a demonstration of the =multiprocessing= library code:
...
Note that with such a simple operation, the time associated with starting processes may outweigh the time saved by multi-processing!
